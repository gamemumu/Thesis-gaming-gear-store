...
<div class="modal-content">
    <div class="modal-header">
        ...
        <h3 id="myModalLabel">รายละเอียดใบส่งสินค้า <br /> <small>รหัสใบส่ง : <%= ViewBag.DivID %>  &nbsp รหัสการขาย : <%= ViewBag.SldID %> </small></h3>
    </div>
    <!--head-->
    <div style="padding: 15px">
        ...
        <table class="table table-striped table-condensed" style="width: 100%">
            ...
            <tbody>
                <% if (ViewData["EModalDelivery"] != null)
                   {%>
                <%foreach (var item in (List<SELLPRODUCT_DTA>)ViewData["EModalDelivery"])
                  {%>
                <tr>
                    <td>
        ...
        <div class="modal-footer ">
            <p><strong>เลขพัสดุ :</strong> <input id="EMS_serial" style="width:113px" placeholder="ใส่เลขพัสดุ" value="<%= ViewBag.EMS %>"/></p>
            <!--footer-->
            <button class="btn btn-default" data-dismiss="modal" aria-hidden="true">ปิด</button>
            <button class="btn btn-inverse btn-primary" type="submit" onclick="EditDeliver('<%= ViewBag.DivID %>','<%= ViewBag.SldID %>')">แก้ไขเลขพัศดุ</button>
        </div>
        <!--footer-->
    </div>
</div>
