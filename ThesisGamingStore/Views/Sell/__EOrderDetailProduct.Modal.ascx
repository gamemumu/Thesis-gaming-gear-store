    ...
    <div style="padding:15px">
        ...
        <%--<div class="table-responsive cart_info">--%>
             <table class="table table-striped table-condensed" style="width:100%">
                ...
                <tbody>
                    <% if (ViewData["EDetailOrder"] != null)
                       {%>
                    <%foreach (var item in (List<SELLPRODUCT_DTA>)ViewData["EDetailOrder"])
                      {%>
                    <tr>
                        <td>
                               <a href="<%=item.PRODUCT.PHOTO.FirstOrDefault().Photo1%>" data-toggle="lightbox" data-title="<%=item.PrdID %> &nbsp<%= item.PRODUCT.BRAND.TRADEMARK.TrdName %> &nbsp <%=item.PRODUCT.BRAND.BrandName %>">
                                <img src="<%=item.PRODUCT.PHOTO.FirstOrDefault().Photo1%>" class="img-responsive">
                            </a>
                        </td>
                        <td>
