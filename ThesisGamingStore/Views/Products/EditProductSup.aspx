...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h3 class="page-header">ราคาทุนของสินค้าต่อบริษัทคู่ค้า (แก้ไข)</h3>
    ...
    <br />
    <a href="<%= Url.Action("OfferCreate","Products") %>" class="btn btn-inverse btn-info" id="btnList" onclick="btnList()">เสนอสินค้า &nbsp<i class="fa fa-arrow-circle-right fa-lg"> &nbsp</i></a>
</asp:Content>
    ...
    <script type="text/javascript">
        ...
        function getData(sup) {
            if ($("#SupID").val() != 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "<%= Url.Action("JsonProductEditSup","Products") %>", // we are calling json method
                    dataType: 'json',
                    data: { id: sup },
                    success: function (obj) {
                        ...
                        if (parseJSONResult != null && parseJSONResult.length > 0) {
                            ...
                            $.each(parseJSONResult, function (key, value) {
                                ...
                                $.each(parseJSONResult[i], function (key, value) {
                                    if (key == "PrdID")
                                        ...
                                        rowData[8] = numeral(value).format('0,0');
                                    else if (key == "Price") {
                                        rowData[9] = '<input type="number" style="width:100%;text-align:right;" value="' + value + '" onkeypress="return isNumberKey(event);" min= "0" name="Cost[]" id="N' + rowData[0] + '" />';
                                        rowData[10] = '<button id="btnSendData" class="btn btn-inverse btn-primary" onclick="btnSendData(\'' + rowData[0] + '\',\'' + $("#SupID").val() + '\')">ยืนยันราคา</button> ';
                                        rowData[11] = '<i id="S' + rowData[0] + '"style="color:green; visibility:hidden;" class="fa fa-check-square-o fa-lg"></i>';
                                    }
                                });
                            ...
                            $('#example').dataTable({
                                ...
                                "aoColumns": [
                              // { "sWidth": "3%", "sTitle": "#" },
                               { "sWidth": "9%", "sTitle": "Product ID" },
                               { "sWidth": "10%", "sTitle": "ประเภทสินค้า" },
                               { "sWidth": "10%", "sTitle": "ยี่ห้อ" },
                               { "sWidth": "20%", "sTitle": "รุ่นสินค้า" },
                               { "sWidth": "5%", "sTitle": "สี" },
                               { "sWidth": "10%", "sTitle": "สถานะสินค้า" },
                               { "sWidth": "3%", "sTitle": "Photo" },
                               { "sWidth": "7%", "sTitle": "อัพเดทล่าสุด" },
                               { "sWidth": "8%", "sTitle": "รายคาขาย", "sClass": "right" },
        ...
        function btnSendData(prd, sup) {
            var price = ($('#N' + prd).val());
            var cost = numeral().unformat($('#N' + prd).closest("tr").find('td:eq(8)').text());
            ...
            if (_price > 0 && _price <= cost) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<%= Url.Action("ProductSupEdit","Products")%>",
                    dataType: "json",
