...
function _price(array) {
    return $.ajax({
        type: 'POST',
        url: "/Shipping/JsonCreateCookie",
        dataType: 'json',
        data: { id: array[0].split('=')[1], status: array[2].split('=')[1] },
...
/* PUBLIC: */
	...
	*/
	...
	this.add = function () {
	    ...
	    /////////////////////////////////////////////////
		var newItem = new CartItem(arguments);
		this.id = arguments[0].split('=')[1];
		newItem.parseValuesFromArray(arguments);
		newItem.checkQuantityAndPrice(arguments);
		
		$.ajax({
		    type: 'POST',
		    url: "/Shipping/JsonCreateCookie",
		    dataType: 'json',
		    data: { id: arguments[0].split('=')[1], status: arguments[2].split('=')[1] },
		    success: function (data) {
		        $.each(data, function (i, obj) {
		            newItem.price = (obj.Price - (obj.Price * (obj.Discount / 100)));
		        });
		        this.update();
		    },
		});
